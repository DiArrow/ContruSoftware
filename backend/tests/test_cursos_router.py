...
class TestListarCursos:
    ...
    def test_pro_ve_solo_sus_cursos(self, client, db_session):
        ...
        curso_otro = Curso(id_curso=str(uuid4()), nombre="Curso ajeno",
                           ref_semestre=otro_semestre_id, ref_profesor=otro_pro_id)
...
class TestActualizarCurso:
    ...
    def test_happy_path_adm(self, client, db_session):
        ...
        response = client.put(f"/api/cursos/{curso_id}", json={"nombre": "Actualizado"},
                              headers=headers)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_403_no_adm(self, client_unit, pro_headers):
        """Rol no ADM → 403."""
        response = client_unit.put(f"/api/cursos/{uuid4()}", json={"nombre": "x"},
                                   headers=pro_headers)
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_404_no_encontrado(self, client, db_session):
        """ID inexistente → 404."""
        headers = _token_headers("ADM", str(USER_IDS["ADM"]))
        response = client.put(f"/api/cursos/{uuid4()}", json={"nombre": "x"},
                              headers=headers)
        assert response.status_code == status.HTTP_404_NOT_FOUND
